...
5. **Hash-only tokens remain.** Raw tokens appear only in the one-time URL at send time. `mail_outbox.action_url` is column-revoked from `schoolapp_app` and wiped after send. Admin GET redacts `token=`.
6. **Idempotency keys** prevent duplicate invitation, reset, and admissions-acknowledgement sends.

## Consequences

- Migration `0048_email_delivery.sql` is additive on `mail_outbox`. It does not rewrite 0047 or earlier.
