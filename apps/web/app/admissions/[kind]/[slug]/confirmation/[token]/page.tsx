...
export default function PublicAdmissionsConfirmationPage() {
  ...
  if (!formType || !params.slug || !token) {
    return (
      <main className="admissions-app">
        <h1>Confirmation unavailable</h1>
        <p>This confirmation is no longer available.</p>
      </main>
    );
  }
  return <PublicAdmissionsConfirmation formType={formType} slug={params.slug} token={token} />;
}
