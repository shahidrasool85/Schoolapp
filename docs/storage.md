...
Postgres and the object store are not one transaction. Failed writes mark the row `rejected`. `pnpm storage:cleanup` removes expired pending objects and old rejected/deleted bytes. It never auto-deletes safeguarding objects.
