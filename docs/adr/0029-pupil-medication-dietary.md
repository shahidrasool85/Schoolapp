...
4. **Parent Portal is opt-in per record, fail-closed, and rechecked.** `students.medications.read_own_children` / `students.dietary.read_own_children` plus live `guardianship` + `portal_access` (not ended) are required. Only `parent_visible` rows are returned. Knowing a child id is not enough. Ended or portal-off guardianships 404. Create paths treat omitted or non-true `parentVisible` as false; the database default is false. Staff must explicitly set `parentVisible: true`. PATCH omits leave the stored flag unchanged. Admissions conversion seeds structured rows as staff-only so form text is not silently published to parents. Existing rows are not bulk-updated.
...
- Admissions conversion still writes legacy text and, when present, seeds one structured medication/dietary row from that text as staff-only (`parent_visible = false`) until authorised staff opt the record in.
