...
- AI learning, PDF report cards, and mobile apps are **not built yet**. Formal assessments/results/reports are in Phase 8. Public admissions forms are in Phase 9. Announcements and the school calendar are in Phase 10. The school timetable is in Phase 12. Trips, clubs, and parent consent are in Phase 14. School charges and parent payments are in Phase 15 (local fake provider; no Stripe credentials required). Parent–teacher messaging is in Phase 16 (in-app only; no email/SMS). Phase 13 object storage is available for pupil, learning, pastoral, safeguarding, admissions, activity, and message files.
